...
{
 if(/^[a-zA-Z]\w{2,9}$/i.test(str))   //用户名由字母和数字、下划线组成，且只能以字母开头，且长度最小为4位
...
//--------------------------------判断用户名（判断字符由字母和数字,汉字，下划线,点号组成.且开头的只能是下划线和字母）
function isName(str)
{
 var pattern = /^[a-zA-Z0-9\u4e00-\u9fa5][a-zA-Z0-9_\u4e00-\u9fa5]{2,19}$/i;
 if (pattern.test(str))
 {
  return true;
 }
...
//--------------------------------判断密码（ 能匹配的组合为：数字+字母，数字+特殊字符，字母+特殊字符，数字+字母+特殊字符组合，而且不能是纯数字，纯字母，纯特殊字符(!#$%^&*_.)）
function isPwd(str)
{
	//var pattern = /^(?![a-zA-Z]+$)(?![\d]+$)(?![@#$%^&_~!*]+$)[\da-zA-Z@#$%^&_~!*]{6,12}$/i;
	var pattern = /^(?=.*[a-zA-Z])(?=.*\d)(?=.*[~!@#$%^&*()_+`\-={}:";'<>?,.\/]).{6,12}$/; 
	if (pattern.test(str))
	{
		return true;
	}
...
{     
 ...
 var reg=/^1((3[0-9]|4[57]|5[0-3,5-9]|7[3678]|8[0-9])\d{8}|(70[0-9]|71[0-9])\d{7})$/;
...
{
 var obj=document.getElementByIdx(objname)
